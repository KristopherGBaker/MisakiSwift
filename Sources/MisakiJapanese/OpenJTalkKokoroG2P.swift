...
/// One phonemized token, light enough for every platform (the MToken adapter
/// for the Apple Kokoro pipeline lives in MisakiSwift). `whitespace` is mutable
/// because accent-phrase separation is decided by the NEXT token.
public final class KokoroPhonemeToken {
    public let text: String
    public var whitespace: String
    public let phonemes: String?

    public init(text: String, whitespace: String, phonemes: String?) {
        self.text = text
...
public final class OpenJTalkKokoroG2P {
    ...
    public func phonemize(text: String) -> (String, [KokoroPhonemeToken]) {
    ...
    private func makeTokens(_ words: [AnalyzedWord], in text: String) -> [KokoroPhonemeToken] {
        var tokens: [KokoroPhonemeToken] = []
        ...
        for word in words {
            if let phonemes = word.phonemes, !phonemes.isEmpty {
                ...
                tokens.append(KokoroPhonemeToken(
                    text: word.surface, whitespace: whitespace, phonemes: phonemes))
            } else if let mapped = Self.punctuation(word.surface) {
                if let last = tokens.last { last.whitespace = "" }
                tokens.append(KokoroPhonemeToken(
                    text: word.surface, whitespace: " ", phonemes: mapped))
            }
